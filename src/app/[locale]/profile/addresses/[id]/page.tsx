...
import { Form, Input, Button, Card, Checkbox, Select, message, Spin } from 'antd';
...
export default function EditAddressPage() {
  ...
  const loadAddress = async () => {
    setDataLoading(true);
    const result = await apiGet(`/api/user/addresses/${id}`);
    if (result.success && result.data?.address) {
      const address = result.data.address;
      form.setFieldsValue(address);
      setSelectedCountry(address.country);
    } else {
      message.error(locale === 'zh' ? '地址不存在' : 'Address not found');
  ...
  const handleSubmit = async (values: any) => {
    setLoading(true);
    try {
      const result = await apiPut(`/api/user/addresses/${id}`, values, {
        showError: true,
        showSuccess: true,
        successMessage: locale === 'zh' ? '地址更新成功' : 'Address updated successfully'
      });
